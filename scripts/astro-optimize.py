...
Usage (Astro must be running):
  ...
  ./scripts/astro-optimize.py --all-stores --prune
"""
...
from astro_locale import is_headache_junk, is_english_bleed, is_wrong_script, should_prune
...
# Cross-app / test junk (all stores)
DROP_GLOBAL = {
...
def classify(k: dict, store: str) -> str:
    kw = k["keyword"]
    kl = kw.lower()
    if kl in DROP_GLOBAL or is_headache_junk(kw) or should_prune(kw, store):
        return "drop"
...
def drop_set_for_store(store: str, extra: list[str]) -> set[str]:
    return {x.strip().lower() for x in extra if x.strip()} | DROP_GLOBAL
...
def main() -> None:
    ...
    parser.add_argument("--prune-list", nargs="*", default=[])
    ...
    if args.all_stores:
        ...
        for i, store in enumerate(stores):
            try:
                ...
                if args.prune:
                    to_remove = [k["keyword"] for k in data["portfolio"]["drop"]]
                    if to_remove:
                        remove_keywords(MCP_URL, app_id, store, to_remove)
                        print(f"{store}: pruned {len(to_remove)}")
                    else:
                        print(f"{store}: nothing to prune")
                else:
    ...
    if args.prune:
        ...
        for tier in data["portfolio"].values():
            for k in tier:
                kl = k["keyword"].lower()
                if kl in drop_set_for_store(store, args.prune_list):
                    to_remove.append(k["keyword"])
        ...
        for kw in to_remove[:20]:
            print(f"  - {kw}")
        if len(to_remove) > 20:
            print(f"  ... +{len(to_remove)-20} more")
