...
from astro_locale import ENGLISH_ASTRO_STORES, locale_dirs_for_store
...
def build_keywords_for_locales(locale_dirs: list[Path], store_code: str) -> list[str]:
    ...
    elif store_code in ENGLISH_ASTRO_STORES:
        items.extend(
            [
                "habit streak",
                "streak tracker",
...
def build_store_plan(app_id: str) -> dict[str, dict]:
    ...
    for entry in load_all_stores():
        code = entry["code"]
        dirs = locale_dirs_for_store(code, entry)  # native-only per store (see astro_locale.py)
