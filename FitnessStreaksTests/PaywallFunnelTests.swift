...
final class PaywallFunnelTests: XCTestCase {
    ...
    func testCountsPitchesPerSurfaceAndInTotal() {
        ConversionDiagnostics.recordPitchView(impressionID: "streaks_dashboard_sheet")
        ConversionDiagnostics.recordPitchView(impressionID: "streaks_dashboard_sheet")
        ConversionDiagnostics.recordPitchView(impressionID: "streaks_detail_sheet")

        let attributes = ConversionDiagnostics.subscriberAttributes
        XCTAssertEqual(attributes["pitch_views_total"], "3")
        XCTAssertEqual(attributes["pitch_views_dashboard_sheet"], "2")
        XCTAssertEqual(attributes["pitch_last"], "detail_sheet")
    }
    ...
    func testConversionFreezesTheSurfaceAndCountAtTheMomentOfSale() {
        ...
        let attributes = ConversionDiagnostics.subscriberAttributes
        XCTAssertEqual(attributes["converted_surface"], "detail_sheet")
    ...
    func testAttributeKeysStayInsideRevenueCatsLimit() {
        ...
        ConversionDiagnostics.recordPitchView(impressionID: "streaks_\(long)")
