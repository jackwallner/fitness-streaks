...
struct StreakTimelineProvider: TimelineProvider {
    ...
    func getTimeline(in context: Context, completion: @escaping @Sendable (Timeline<StreakEntry>) -> Void) {
        ...
        let next = min(nextHour, tomorrow)
        completion(Timeline(entries: [entry], policy: .after(next)))
    }

    private func currentEntry() -> StreakEntry {
        guard let snap = SnapshotStore.load() else {
            return StreakEntry(date: .now, hero: nil, badges: [])
        }
        return StreakEntry(date: .now, hero: snap.hero, badges: snap.badges)
    }
}

// MARK: - Helpers

private func metric(from raw: String) -> StreakMetric? { StreakMetric(rawValue: raw) }
private func cadence(from raw: String) -> StreakCadence { StreakCadence(rawValue: raw) ?? .daily }
...
struct StreakWidgetView: View {
    ...
    private var inlineView: some View {
        if let hero = entry.hero, let m = metric(from: hero.metric) {
            return AnyView(
    ...
    private var rectangularView: some View {
        VStack(alignment: .leading, spacing: 2) {
            if let hero = entry.hero, let m = metric(from: hero.metric) {
                HStack(spacing: 4) {
                    Image(systemName: m.symbol).font(.system(size: 11, weight: .semibold))
                ...
                Text(m.thresholdLabel(hero.threshold, cadence: cadence(from: hero.cadence)))
